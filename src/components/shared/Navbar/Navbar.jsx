      ...
      <div className='container mx-auto px-4 flex justify-between items-center'>
        ...
        <ul className='md:flex gap-4 items-center hidden'>
          {icons?.map((icon) => (
            <li
              className={`${
                icon !== userProfileIcon && 'bg-[#2B2B2B] w-10 h-10 rounded-full p-1'
        ...
        <img src={userProfileIcon} alt="Icon" />
            <button className='text-white bg-[#2B2B2B] w-10 h-10 rounded-full p-1 flex items-center justify-center text-2xl' 
        ...
        {toggle && 
            ...
            {icons?.map((icon) => (
              <li
                key={icon.id}
                className={`${icon.img === userProfileIcon && 'hidden'} 
                           cursor-pointer
                           flex items-center 
                           justify-between hover:bg-[#2B2B2B]
                           px-8 py-2
                           rounded-lg
                           gap-6
